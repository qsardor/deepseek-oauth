...
async function handleRequest(
  ...
  if (path === "/v1/chat/completions" || path === "/chat/completions") {
    ...
    const existingSessionId = req.headers["x-deepseek-chat-session-id"] as string | undefined;
    ...
    // Forward all RedSeek specific headers (like x-redseek-session) to the transport layer
    for (const [key, value] of Object.entries(req.headers)) {
      if (key.toLowerCase().startsWith("x-redseek-") && typeof value === "string") {
